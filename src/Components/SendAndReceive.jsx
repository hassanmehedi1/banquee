import React from 'react';
import Aos from "aos";
import "aos/dist/aos.css";
import { useEffect } from "react";

const SendAndReceive = () => {
  useEffect(() => {
    Aos.init({ duration: 2000 });
  }, []);
   return (
     <div class="bg-secondly w-full">
       <div class="px-6 py-16 mx-auto">
         <div class="items-center lg:flex w-full">
           <div class="w-full lg:w-1/2 lg:ml-24">
             <div
               class="lg:max-w-lg text-center lg:text-start"
               data-aos="fade-right"
             >
               <h1 class="text-3xl font-semibold text-gray-800  lg:text-6xl">
                 Send and receive <br /> money instantly
               </h1>

               <p class="mt-4 text-xl">
                 Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
                 eiusmod tempor incididunt ut labore et.
               </p>

               <div class="grid grid-cols-1 mt-8 gap-y-4 justify-items-center lg:justify-items-start">
                 <div className="flex justify-center items-center space-x-2 mr-3">
                   <svg
                     width="26"
                     height="27"
                     viewBox="0 0 26 27"
                     fill="none"
                     xmlns="http://www.w3.org/2000/svg"
                   >
                     <rect
                       y="0.723633"
                       width="26"
                       height="26"
                       rx="13"
                       fill="#5BB5A2"
                     />
                     <path
                       d="M11.5 16.1028L18.394 9.20801L19.4553 10.2685L11.5 18.2238L6.72701 13.4508L7.78751 12.3903L11.5 16.1028Z"
                       fill="white"
                     />
                   </svg>
                   <p>Malesuada Ipsum</p>
                 </div>
                 <div className="flex justify-center items-center space-x-2 mr-14">
                   <svg
                     width="26"
                     height="27"
                     viewBox="0 0 26 27"
                     fill="none"
                     xmlns="http://www.w3.org/2000/svg"
                   >
                     <rect
                       y="0.723633"
                       width="26"
                       height="26"
                       rx="13"
                       fill="#5BB5A2"
                     />
                     <path
                       d="M11.5 16.1028L18.394 9.20801L19.4553 10.2685L11.5 18.2238L6.72701 13.4508L7.78751 12.3903L11.5 16.1028Z"
                       fill="white"
                     />
                   </svg>
                   <p>Vestibulum</p>
                 </div>
                 <div className="flex justify-center items-center space-x-2 mr-4">
                   <svg
                     width="26"
                     height="27"
                     viewBox="0 0 26 27"
                     fill="none"
                     xmlns="http://www.w3.org/2000/svg"
                   >
                     <rect
                       y="0.723633"
                       width="26"
                       height="26"
                       rx="13"
                       fill="#5BB5A2"
                     />
                     <path
                       d="M11.5 16.1028L18.394 9.20801L19.4553 10.2685L11.5 18.2238L6.72701 13.4508L7.78751 12.3903L11.5 16.1028Z"
                       fill="white"
                     />
                   </svg>
                   <p>Parturient Lorem</p>
                 </div>
               </div>
             </div>
           </div>

           <section className="lg:w-1/2 gap-y-2" data-aos="fade-left">
             <div class="flex items-center justify-between mt-6 lg:mt-0  p-2 shadow-lg w-full rounded-lg bg-white mb-2 ">
               <div
                 className="flex justify-center items-center gap-x-2"
                 data-aos="fade-up"
               >
                 <svg
                   width="50"
                   height="51"
                   viewBox="0 0 50 51"
                   fill="none"
                   xmlns="http://www.w3.org/2000/svg"
                 >
                   <rect y="0.5" width="50" height="50" rx="8" fill="#5BB5A2" />
                   <path
                     d="M24.5613 19.9257C23.5393 19.9257 21.9573 18.7637 20.2913 18.8057C18.0933 18.8337 16.0773 20.0797 14.9433 22.0537C12.6613 26.0157 14.3553 31.8677 16.5813 35.0877C17.6733 36.6557 18.9613 38.4197 20.6693 38.3637C22.3073 38.2937 22.9233 37.2997 24.9113 37.2997C26.8853 37.2997 27.4453 38.3637 29.1813 38.3217C30.9453 38.2937 32.0653 36.7257 33.1433 35.1437C34.3893 33.3237 34.9073 31.5597 34.9353 31.4617C34.8933 31.4477 31.5053 30.1457 31.4633 26.2257C31.4353 22.9497 34.1373 21.3817 34.2633 21.3117C32.7233 19.0577 30.3573 18.8057 29.5313 18.7497C27.3753 18.5817 25.5693 19.9257 24.5613 19.9257ZM28.2013 16.6217C29.1113 15.5297 29.7133 14.0037 29.5453 12.4917C28.2433 12.5477 26.6753 13.3597 25.7373 14.4517C24.8973 15.4177 24.1693 16.9717 24.3653 18.4557C25.8073 18.5677 27.2913 17.7137 28.2013 16.6217Z"
                     fill="white"
                   />
                 </svg>
                 <div className="">
                   <h2 className="text-lg font-semibold">Apple</h2>
                   <p className="text-gray-400">Macbook</p>
                 </div>
               </div>
               <div className="ml-48 font-semibold">
                 <p>-999€</p>
               </div>
             </div>
             <div class="flex items-center justify-between mt-6 lg:mt-0  p-2 shadow-lg w-full rounded-lg bg-white mb-2">
               <div
                 className="flex justify-center items-center gap-x-2"
                 data-aos="fade-up"
               >
                 <svg
                   width="50"
                   height="51"
                   viewBox="0 0 50 51"
                   fill="none"
                   xmlns="http://www.w3.org/2000/svg"
                 >
                   <rect y="0.5" width="50" height="50" rx="8" fill="#5BB5A2" />
                   <path
                     d="M24.5613 19.9257C23.5393 19.9257 21.9573 18.7637 20.2913 18.8057C18.0933 18.8337 16.0773 20.0797 14.9433 22.0537C12.6613 26.0157 14.3553 31.8677 16.5813 35.0877C17.6733 36.6557 18.9613 38.4197 20.6693 38.3637C22.3073 38.2937 22.9233 37.2997 24.9113 37.2997C26.8853 37.2997 27.4453 38.3637 29.1813 38.3217C30.9453 38.2937 32.0653 36.7257 33.1433 35.1437C34.3893 33.3237 34.9073 31.5597 34.9353 31.4617C34.8933 31.4477 31.5053 30.1457 31.4633 26.2257C31.4353 22.9497 34.1373 21.3817 34.2633 21.3117C32.7233 19.0577 30.3573 18.8057 29.5313 18.7497C27.3753 18.5817 25.5693 19.9257 24.5613 19.9257ZM28.2013 16.6217C29.1113 15.5297 29.7133 14.0037 29.5453 12.4917C28.2433 12.5477 26.6753 13.3597 25.7373 14.4517C24.8973 15.4177 24.1693 16.9717 24.3653 18.4557C25.8073 18.5677 27.2913 17.7137 28.2013 16.6217Z"
                     fill="white"
                   />
                 </svg>
                 <div className="">
                   <h2 className="text-lg font-semibold">Amazon</h2>
                   <p className="text-gray-400">Electronics</p>
                 </div>
               </div>
               <div className="ml-48 font-semibold">
                 <p>-49€</p>
               </div>
             </div>
             <div class="flex items-center justify-between mt-6 lg:mt-0  p-2 shadow-lg w-full rounded-lg bg-white mb-2">
               <div
                 className="flex justify-center items-center gap-x-2"
                 data-aos="fade-up"
               >
                 <svg
                   width="50"
                   height="51"
                   viewBox="0 0 50 51"
                   fill="none"
                   xmlns="http://www.w3.org/2000/svg"
                 >
                   <rect y="0.5" width="50" height="50" rx="8" fill="#5BB5A2" />
                   <path
                     d="M24.5613 19.9257C23.5393 19.9257 21.9573 18.7637 20.2913 18.8057C18.0933 18.8337 16.0773 20.0797 14.9433 22.0537C12.6613 26.0157 14.3553 31.8677 16.5813 35.0877C17.6733 36.6557 18.9613 38.4197 20.6693 38.3637C22.3073 38.2937 22.9233 37.2997 24.9113 37.2997C26.8853 37.2997 27.4453 38.3637 29.1813 38.3217C30.9453 38.2937 32.0653 36.7257 33.1433 35.1437C34.3893 33.3237 34.9073 31.5597 34.9353 31.4617C34.8933 31.4477 31.5053 30.1457 31.4633 26.2257C31.4353 22.9497 34.1373 21.3817 34.2633 21.3117C32.7233 19.0577 30.3573 18.8057 29.5313 18.7497C27.3753 18.5817 25.5693 19.9257 24.5613 19.9257ZM28.2013 16.6217C29.1113 15.5297 29.7133 14.0037 29.5453 12.4917C28.2433 12.5477 26.6753 13.3597 25.7373 14.4517C24.8973 15.4177 24.1693 16.9717 24.3653 18.4557C25.8073 18.5677 27.2913 17.7137 28.2013 16.6217Z"
                     fill="white"
                   />
                 </svg>
                 <div className="">
                   <h2 className="text-lg font-semibold">Twitter</h2>
                   <p className="text-gray-400">Ads</p>
                 </div>
               </div>
               <div className="ml-48 font-semibold">
                 <p>-29€</p>
               </div>
             </div>
             <div class="flex items-center justify-between mt-6 lg:mt-0  p-2 shadow-lg w-full rounded-lg bg-white mb-2">
               <div
                 className="flex justify-center items-center gap-x-2"
                 data-aos="fade-up"
               >
                 <svg
                   width="50"
                   height="51"
                   viewBox="0 0 50 51"
                   fill="none"
                   xmlns="http://www.w3.org/2000/svg"
                 >
                   <rect y="0.5" width="50" height="50" rx="8" fill="#5BB5A2" />
                   <path
                     d="M24.5613 19.9257C23.5393 19.9257 21.9573 18.7637 20.2913 18.8057C18.0933 18.8337 16.0773 20.0797 14.9433 22.0537C12.6613 26.0157 14.3553 31.8677 16.5813 35.0877C17.6733 36.6557 18.9613 38.4197 20.6693 38.3637C22.3073 38.2937 22.9233 37.2997 24.9113 37.2997C26.8853 37.2997 27.4453 38.3637 29.1813 38.3217C30.9453 38.2937 32.0653 36.7257 33.1433 35.1437C34.3893 33.3237 34.9073 31.5597 34.9353 31.4617C34.8933 31.4477 31.5053 30.1457 31.4633 26.2257C31.4353 22.9497 34.1373 21.3817 34.2633 21.3117C32.7233 19.0577 30.3573 18.8057 29.5313 18.7497C27.3753 18.5817 25.5693 19.9257 24.5613 19.9257ZM28.2013 16.6217C29.1113 15.5297 29.7133 14.0037 29.5453 12.4917C28.2433 12.5477 26.6753 13.3597 25.7373 14.4517C24.8973 15.4177 24.1693 16.9717 24.3653 18.4557C25.8073 18.5677 27.2913 17.7137 28.2013 16.6217Z"
                     fill="white"
                   />
                 </svg>
                 <div className="">
                   <h2 className="text-lg font-semibold">Microsoft</h2>
                   <p className="text-gray-400">Office Suite</p>
                 </div>
               </div>
               <div className="ml-48 font-semibold">
                 <p>-149€</p>
               </div>
             </div>
             <div class="flex items-center justify-between mt-6 lg:mt-0  p-2 shadow-lg w-full rounded-lg bg-white mb-2">
               <div
                 className="flex justify-center items-center gap-x-2"
                 data-aos="fade-up"
               >
                 <svg
                   width="50"
                   height="51"
                   viewBox="0 0 50 51"
                   fill="none"
                   xmlns="http://www.w3.org/2000/svg"
                 >
                   <rect y="0.5" width="50" height="50" rx="8" fill="#5BB5A2" />
                   <path
                     d="M24.5613 19.9257C23.5393 19.9257 21.9573 18.7637 20.2913 18.8057C18.0933 18.8337 16.0773 20.0797 14.9433 22.0537C12.6613 26.0157 14.3553 31.8677 16.5813 35.0877C17.6733 36.6557 18.9613 38.4197 20.6693 38.3637C22.3073 38.2937 22.9233 37.2997 24.9113 37.2997C26.8853 37.2997 27.4453 38.3637 29.1813 38.3217C30.9453 38.2937 32.0653 36.7257 33.1433 35.1437C34.3893 33.3237 34.9073 31.5597 34.9353 31.4617C34.8933 31.4477 31.5053 30.1457 31.4633 26.2257C31.4353 22.9497 34.1373 21.3817 34.2633 21.3117C32.7233 19.0577 30.3573 18.8057 29.5313 18.7497C27.3753 18.5817 25.5693 19.9257 24.5613 19.9257ZM28.2013 16.6217C29.1113 15.5297 29.7133 14.0037 29.5453 12.4917C28.2433 12.5477 26.6753 13.3597 25.7373 14.4517C24.8973 15.4177 24.1693 16.9717 24.3653 18.4557C25.8073 18.5677 27.2913 17.7137 28.2013 16.6217Z"
                     fill="white"
                   />
                 </svg>
                 <div className="">
                   <h2 className="text-lg font-semibold">Dropbox</h2>
                   <p className="text-gray-400">Cloud</p>
                 </div>
               </div>
               <div className="ml-48 font-semibold">
                 <p>-14€</p>
               </div>
             </div>
             <div class="flex items-center justify-between mt-6 lg:mt-0  p-2 shadow-lg w-full rounded-lg bg-white mb-2">
               <div
                 className="flex justify-center items-center gap-x-2"
                 data-aos="fade-up"
               >
                 <svg
                   width="50"
                   height="51"
                   viewBox="0 0 50 51"
                   fill="none"
                   xmlns="http://www.w3.org/2000/svg"
                 >
                   <rect y="0.5" width="50" height="50" rx="8" fill="#5BB5A2" />
                   <path
                     d="M24.5613 19.9257C23.5393 19.9257 21.9573 18.7637 20.2913 18.8057C18.0933 18.8337 16.0773 20.0797 14.9433 22.0537C12.6613 26.0157 14.3553 31.8677 16.5813 35.0877C17.6733 36.6557 18.9613 38.4197 20.6693 38.3637C22.3073 38.2937 22.9233 37.2997 24.9113 37.2997C26.8853 37.2997 27.4453 38.3637 29.1813 38.3217C30.9453 38.2937 32.0653 36.7257 33.1433 35.1437C34.3893 33.3237 34.9073 31.5597 34.9353 31.4617C34.8933 31.4477 31.5053 30.1457 31.4633 26.2257C31.4353 22.9497 34.1373 21.3817 34.2633 21.3117C32.7233 19.0577 30.3573 18.8057 29.5313 18.7497C27.3753 18.5817 25.5693 19.9257 24.5613 19.9257ZM28.2013 16.6217C29.1113 15.5297 29.7133 14.0037 29.5453 12.4917C28.2433 12.5477 26.6753 13.3597 25.7373 14.4517C24.8973 15.4177 24.1693 16.9717 24.3653 18.4557C25.8073 18.5677 27.2913 17.7137 28.2013 16.6217Z"
                     fill="white"
                   />
                 </svg>
                 <div className="">
                   <h2 className="text-lg font-semibold">Paypal</h2>
                   <p className="text-gray-400">Shopping</p>
                 </div>
               </div>
               <div className="ml-48 font-semibold">
                 <p>-200€</p>
               </div>
             </div>
           </section>
         </div>
       </div>
     </div>
   );
};

export default SendAndReceive;